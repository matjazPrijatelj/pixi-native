...
class FakeTicker {
  ...
  public update(): void {
    const entries = [...this.entries].sort((a, b) => b.priority - a.priority);
...
test("managed runtime forwards pointer, wheel, keyboard, and resize", async () => {
  ...
  assert.equal(
    (fixture.canvasEvents[0].event as unknown as { buttons: number }).buttons,
    1,
  );
  assert.equal(
  ...
  assert.equal(
    (fixture.globalEvents[1].event as unknown as { buttons: number }).buttons,
    0,
  );
  assert.equal(
    (fixture.globalEvents[2].event as unknown as { ctrlKey: boolean }).ctrlKey,
    true,
  );
