...
test("quickboot parses explicit Pixi and backend options", () => {
  ...
  assert.equal(
    parseCliArguments(["display", "--animation=gsap"]).animation,
    "gsap",
  );
});

test("quickboot keeps generator and runtime versions independent", () => {
  assert.equal(GENERATOR_VERSION, "0.1.11");
  assert.equal(PIXI_NATIVE_VERSION, "0.2.7");
  assert.equal(PIXI_NATIVE_VERSION_RANGE, "~0.2.7");
  assert.equal(PNPM_VERSION, "12.4.1");
  assert.equal(
    execFileSync(
      process.execPath,
      [
        resolve(REPOSITORY_ROOT, "packages/create-pixi-native/src/cli.ts"),
        "--version",
      ],
      { encoding: "utf8" },
    ).trim(),
...
test("quickboot generates version-specific projects without credentials", async () => {
  ...
  try {
    for (const [
      ...
      assert.deepEqual(await readdir(join(target, "assets")), [
        "pixi-hero.png",
      ]);
      assert.deepEqual(
