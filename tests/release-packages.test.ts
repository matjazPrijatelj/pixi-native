...
test("facade exports root Pixi 8 and explicit version and core paths", async () => {
  ...
  assert.deepEqual(manifest.optionalDependencies, {
    "@matjash/pixi-native-linux-x64": "~0.2.6",
    "@matjash/pixi-native-win32-x64": "~0.2.6",
  });
...
test("private Pixi facades use ranged peers and exact development versions", async () => {
  for (const [packagePath, peerName, peerVersion, developmentVersion] of [
    ...
    ["packages/pixi8/package.json", "pixi.js", "^8.20.0", "8.21.0"],
  ] as const) {
    const manifest = JSON.parse(
      await readFile(new URL(packagePath, REPOSITORY_ROOT), "utf8"),
    );
    assert.equal(manifest.version, "0.2.6", packagePath);
    assert.equal(manifest.peerDependencies["@pixi-native/core"], "~0.2.6");
...
test("core accepts patched native packages without selecting the old Win32 build", async () => {
  const manifest = JSON.parse(
    await readFile(
      new URL("packages/core/package.json", REPOSITORY_ROOT),
      "utf8",
    ),
  );
...
test("WSL release commands use temporary LF scripts", async () => {
  ...
  assert.doesNotMatch(source, /pnpm native:window:build/);
  assert.match(source, /for component in gpu window audio video/);
  assert.match(source, /cp -a "`\$staged\/\." "`\$development\/"/);
});
...
test("publisher selects package-specific versions, archives, and tags", () => {
  assert.deepEqual(getReleaseConfiguration(true, "0.2.1", "0.1.8"), {
    version: "0.1.8",
    tags: ["v0.2.1", "create-pixi-native-v0.1.8"],
    expectedPackages: [
      "@matjash/pixi-native-win32-x64",
      "@matjash/pixi-native-linux-x64",
      "@matjash/pixi-native",
  ...
  assert.deepEqual(getReleaseConfiguration(false, "0.2.1", "0.1.8"), {
    version: "0.2.1",
    tags: ["v0.2.1"],
    expectedPackages: [
      "@matjash/pixi-native-win32-x64",
  ...
  assert.deepEqual(getReleaseConfiguration(false, "0.2.1", "0.1.8", true), {
    version: "0.2.1",
    tags: ["v0.2.1"],
    expectedPackages: [
      "@matjash/pixi-native-win32-x64",
