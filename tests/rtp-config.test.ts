...
import { loadRtpTestConfig } from "../src/demo/rtpTestConfig.ts";

test("RTP test remains disabled until both local URLs are configured", () => {
    assert.equal(loadRtpTestConfig({}), null);
    assert.equal(loadRtpTestConfig({ RTP_TEST_URL_1: "http://camera-1/stream" }), null);
});
...
test("RTP config builds low-latency FFmpeg input arguments", () => {
    ...
    assert.ok(config);
    assert.deepEqual(config.urls, ["http://camera-1/stream", "http://camera-2/stream"]);
