...
The current generator release is `0.1.9`; both generated Pixi majors target
runtime `0.2.5`.
...
git commit -m "release(npm): publish pixi-native 0.2.5"
git tag -a v0.2.5 -m "pixi-native 0.2.5"
...
requires a clean `v0.2.5` release commit, both platform manifests, all three
...
For the current release, every generator version reference is `0.1.9`; the
separate `PIXI_NATIVE_VERSION` is `0.2.5`.
