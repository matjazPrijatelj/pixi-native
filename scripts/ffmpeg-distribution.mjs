...
export async function writeFfmpegChecksums(directory) {
    ...
    for (const filename of FFMPEG_PACKAGED_FILES) {
        lines.push(
            `${await sha256File(join(directory, filename))}  ${filename}`,
        );
    }
    await writeFile(
...
export async function validateFfmpegIdentity(directory, executeBinaries) {
    ...
    const license = await readFile(
        join(directory, FFMPEG_LICENSE_FILE),
        "utf8",
    );
    ...
    for (const executable of ["ffmpeg.exe", "ffprobe.exe"]) {
        const version = runAndCapture(join(directory, executable), [
            "-version",
        ]);
        ...
        const licenseOutput = runAndCapture(join(directory, executable), [
            "-L",
        ]);
        ...
        if (
            /configuration:.*--enable-(?:gpl|nonfree)/.test(
                normalizedVersion,
            )
        ) {
            throw new Error(
...
export function validateFfmpegCapabilities(directory) {
    ...
    for (const protocol of [
        ...
        if (!protocolNames.has(protocol)) {
            throw new Error(
                `FFmpeg does not provide required protocol: ${protocol}`,
            );
        }
    }
