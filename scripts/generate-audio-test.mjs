...
const outputDirectory = fileURLToPath(
  new URL("../src/demo/assets/audio/", import.meta.url),
);
...
const DRUMS = [
  {
    ...
    duration: 1.4,
    pan: -0.55,
    hit: { x: 0.15, y: 0.13, radiusX: 0.13, radiusY: 0.1 },
  },
  {
    key: "i",
    sprite: "closedHat",
    ...
    duration: 0.35,
    pan: -0.35,
    hit: { x: 0.35, y: 0.17, radiusX: 0.085, radiusY: 0.055 },
  },
  {
    key: "o",
    sprite: "ride",
    ...
    duration: 1.2,
    pan: 0.45,
    hit: { x: 0.63, y: 0.13, radiusX: 0.13, radiusY: 0.1 },
  },
  {
    key: "p",
    sprite: "highTom",
    ...
    duration: 0.75,
    pan: 0.55,
    hit: { x: 0.86, y: 0.17, radiusX: 0.09, radiusY: 0.08 },
  },
  {
    key: "j",
    sprite: "snare",
    ...
    duration: 0.6,
    pan: -0.35,
    hit: { x: 0.23, y: 0.57, radiusX: 0.105, radiusY: 0.085 },
  },
  {
    key: "k",
    sprite: "kick",
    ...
    duration: 0.55,
    pan: 0,
    hit: { x: 0.49, y: 0.68, radiusX: 0.145, radiusY: 0.19 },
  },
  {
    key: "l",
    sprite: "floorTom",
    ...
    duration: 0.85,
    pan: 0.35,
    hit: { x: 0.74, y: 0.62, radiusX: 0.105, radiusY: 0.12 },
  },
  {
    key: "č",
    sprite: "openHat",
    ...
    duration: 1,
    pan: 0.65,
    hit: { x: 0.9, y: 0.48, radiusX: 0.085, radiusY: 0.06 },
  },
];
...
for (let frame = 0; frame < rainDropFrameCount; frame++) {
  ...
  const sample =
    Math.sin(2 * Math.PI * (950 - time * 3800) * time) * envelope * 0.55;
...
for (let frame = 0; frame < 4; frame++) {
  ...
  const gradient = rainContext.createLinearGradient(
    centerX,
    tipY,
    centerX,
    120,
...
await writeFile(
  join(repositoryRoot, "src", "demo", "assets", "rain-drop.png"),
  rainCanvas.toBuffer("image/png"),
);
...
function synthesizeDrum(sprite, time, noise, highNoise) {
  ...
  if (sprite === "crash" || sprite === "ride") {
    ...
    return (
      (highNoise * 0.42 + metallic * 0.18) * attack * Math.exp(-time * decay)
    );
  }
  ...
  const phase =
    Math.PI *
    2 *
    (baseFrequency * time + (frequencyDrop * (1 - Math.exp(-time * 11))) / 11);
  return (
