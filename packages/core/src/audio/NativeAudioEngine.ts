...
export class NativeAudioEngine {
  ...
  public currentTime(id: number): number | undefined {
    ...
    return Math.max(
      0,
      position.seconds -
        ((position.outputFrame - audibleFrame) * position.playbackRate) /
          SAMPLE_RATE,
    );
  }
  ...
  private handleWorkerMessage(message: any): void {
    ...
    const audibleDelayMs =
      (this.device.queued * 1000) / (SAMPLE_RATE * CHANNELS * BYTES_PER_SAMPLE);
...
interface WindowsNativeBindingModule {
  readonly NativeAudioEngine: new (
    eventNotifier: () => void,
  ) => WindowsNativeEngineBinding;
}
...
class WindowsNativeAudioEngine {
  ...
  private ensureStarted(): WindowsNativeEngineBinding {
    ...
    this.native = new binding.NativeAudioEngine(() =>
  ...
  private dispatchPendingEvents(): void {
    ...
    for (const event of events) {
      this.owners
        .get(event.ownerId)
        ?.handleNativeAudioEvent(event.event, event.id, event.message);
    }
  }
}
