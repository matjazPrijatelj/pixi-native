  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once("close", (code) => {
      if (code !== 0) {
        ...
        reject(
          new Error(
            redactUrlCredentials(message) || `FFmpeg exited with code ${code}`,
          ),
        );
        return;
      }
      ...
      cache.set(
        cacheKey(source, offsetSeconds, durationSeconds),
    ...
    if (fullSource) {
      pending = fullSource.then((samples) => {
        ...
        return samples.subarray(
          start,
          Math.min(samples.length, start + length),
        );
      });
...
async function createVoice(
  command: Extract<WorkerCommand, { type: "createVoice" }>,
): Promise<void> {
...
function createStreamingVoice(
  command: Extract<WorkerCommand, { type: "createVoice" }>,
): void {
  const args = ["-hide_banner", "-loglevel", "error", "-nostdin"];
  if (command.offsetSeconds > 0)
    args.push("-ss", String(command.offsetSeconds));
  args.push(...command.inputArgs);
  args.push("-i", command.source, "-vn");
  if (command.durationSeconds !== undefined)
    args.push("-t", String(command.durationSeconds));
  ...
  child.stdout.on("data", (incoming: Buffer) => {
    ...
    const alignedLength =
      data.length - (data.length % (Float32Array.BYTES_PER_ELEMENT * CHANNELS));
    carry =
      alignedLength === data.length
        ? Buffer.alloc(0)
        : data.subarray(alignedLength);
  ...
  child.once("close", (code) => {
    ...
    if (code !== 0) {
      failStreamingVoice(
        voice,
        redactUrlCredentials(Buffer.concat(errors).toString("utf8").trim()) ||
...
function readStreamFrame(
  ...
  while (stream.chunks.length > 0) {
    ...
    if (stream.chunkOffset + 1 < chunk.length) {
      ...
      if (stream.queuedSamples <= STREAM_RESUME_SAMPLES)
...
function applyCommand(
  command: Extract<WorkerCommand, { type: "command" }>,
): void {
  ...
  for (const voice of targets) {
    switch (command.command) {
      ...
      case "loop":
        ...
        break;
      case "seek":
        voice.positionFrames = Math.max(
          0,
          Math.floor(Number(command.value) * SAMPLE_RATE),
        );
        voice.fade = undefined;
      ...
      case "fade":
        ...
        voice.fade = {
          ...
          durationFrames: Math.max(
            1,
            Math.round(((command.durationMs ?? 0) * SAMPLE_RATE) / 1000),
          ),
...
function render(
  ...
  for (let frame = 0; frame < frames; frame++) {
    ...
    for (const voice of [...voices.values()]) {
      ...
      const sourceFrames = voice.data
        ? voice.data.length / CHANNELS
      ...
      if (voice.fade) {
        ...
        const progress = Math.min(
          1,
          Math.max(0, elapsed / voice.fade.durationFrames),
        );
        voice.volume =
          voice.fade.from + (voice.fade.to - voice.fade.from) * progress;
        if (progress >= 1) {
  ...
  const positions = [...voices.values()].map((voice) => ({
    ...
    ownerId: voice.ownerId,
    seconds:
      voice.spriteOffsetSeconds +
      (voice.positionFrames * voice.playbackRate) / SAMPLE_RATE,
  ...
  switch (command.type) {
    ...
    case "unloadOwner": {
      ...
      for (const [id, commands] of pendingVoiceCommands) {
        if (commands[0]?.ownerId === command.ownerId)
          pendingVoiceCommands.delete(id);
      }
      for (const key of ownerCacheKeys.get(command.ownerId) ?? [])
        cache.delete(key);
