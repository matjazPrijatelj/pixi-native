...
const OPTION_EVENTS: ReadonlyArray<
  readonly [keyof HowlOptions, NativeAudioEvent]
> = [
  ["onload", "load"],
...
export class Howl implements NativeAudioEventTarget {
  ...
  public constructor(options: HowlOptions) {
    ...
    for (const [optionName, event] of OPTION_EVENTS) {
      ...
      if (typeof callback === "function")
        this.on(event, callback as HowlEventCallback);
    }
  ...
  public play(spriteOrId?: string | number): number {
    ...
    const definition =
      spriteName === undefined ? undefined : this._sprite[spriteName];
    if (spriteName !== undefined && !definition) {
      this.handleNativeAudioEvent(
        "playerror",
        undefined,
        `Unknown audio sprite: ${spriteName}`,
      );
      return -1;
    }
  ...
  public volume(volume?: number, id?: number): number | this {
    ...
    this.forSounds(id, (sound, soundId) => {
      ...
      nativeAudioEngine.command(this.ownerId, "volume", soundId, {
        value: normalized,
      });
    });
    return this;
  ...
  public mute(muted?: boolean, id?: number): boolean | this {
    ...
    this.forSounds(id, (sound, soundId) => {
      sound.muted = muted;
      nativeAudioEngine.command(this.ownerId, "mute", soundId, {
        value: muted,
      });
    });
    return this;
  ...
  public seek(seekOrId?: number, id?: number): number | this {
    ...
    if (id === undefined && this.sounds.has(seekOrId)) {
      ...
      const offset = sound.sprite
        ? (this._sprite[sound.sprite]?.[0] ?? 0) / 1000
        : 0;
    ...
    this.forSounds(id, (_sound, soundId) => {
      nativeAudioEngine.command(this.ownerId, "seek", soundId, {
        value: seekOrId,
      });
    });
    return this;
  ...
  public fade(from: number, to: number, durationMs: number, id?: number): this {
    ...
    this.forSounds(id, (sound, soundId) => {
      ...
      const fadeFrom =
        sound.fadeTarget === undefined ? normalizedFrom : currentVolume;
  ...
  public duration(id?: number): number {
    ...
    return Math.max(
      0,
      ...Object.values(this._sprite).map(
        ([offset, duration]) => (offset + duration) / 1000,
      ),
    );
  }
