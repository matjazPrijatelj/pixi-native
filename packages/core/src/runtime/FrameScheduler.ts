...
export class VSyncFrameScheduler {
  ...
  public constructor(options: VSyncFrameSchedulerOptions) {
    ...
    this.fallbackFrameIntervalMS = options.fallbackFrameIntervalMS ?? 1000 / 60;
    if (
  ...
  private dispatch(signaled: boolean): void {
    ...
    const elapsed =
      this.lastDispatchTime === undefined
        ? this.minimumFrameSpacingMS
...
export class FrameScheduler {
  ...
  public constructor(options: FrameSchedulerOptions = {}) {
    ...
    this.minimumFrameSpacingRatio = options.minimumFrameSpacingRatio ?? 0.95;
    if (
