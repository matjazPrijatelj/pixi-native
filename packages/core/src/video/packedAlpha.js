...
export function getPackedAlphaVideoLayout(frameWidth, frameHeight, alphaMaskScale) {
    if (!Number.isSafeInteger(frameWidth) ||
        !Number.isSafeInteger(frameHeight) ||
        frameWidth <= 0 ||
        frameHeight <= 0 ||
        frameWidth % 2 !== 0 ||
        frameHeight % 2 !== 0) {
        throw new RangeError("packed alpha NV12 frame dimensions must be positive even integers");
    }
    ...
    if (Math.abs(exactColorWidth - colorWidth) > INTEGER_TOLERANCE) {
        throw new RangeError("alphaMaskScale must split the decoded frame into whole pixel widths");
    }
    if (colorWidth <= 0 ||
        alphaWidth <= 0 ||
        colorWidth % 2 !== 0 ||
        alphaWidth % 2 !== 0) {
        throw new RangeError("packed alpha color and mask widths must be positive even numbers");
    }
    return {
        frameWidth,
        frameHeight,
