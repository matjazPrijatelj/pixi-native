...
function isImageSource(value: unknown): value is WebGlImageSource {
    ...
    return Number.isInteger(source.width) && Number.isInteger(source.height) &&
...
function readImagePixels(
    ...
    const premultiply = Boolean(gl.getParameter(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL));
    ...
    if (premultiply && source.getPremultipliedRgbaPixels) {
        ...
        const context = source.getContext?.("2d") as Canvas2DReadContext | undefined;
        const canvasPixels = context?.getImageData(0, 0, source.width, source.height).data;
        if (canvasPixels) {
            pixels = new Uint8Array(canvasPixels.buffer, canvasPixels.byteOffset, canvasPixels.byteLength);
        } else if (source.data) {
            pixels = new Uint8Array(expectedBytes);
            copyRgbaRowsFlippedY(pixels, source.data, source.width, source.height);
            ...
            fallbackContext.drawImage(source, 0, 0, source.width, source.height);
            if (premultiply) {
                pixels = canvas.getPremultipliedRgbaPixels();
                alreadyPremultiplied = true;
            } else {
                const fallbackPixels = fallbackContext.getImageData(0, 0, source.width, source.height).data;
                pixels = new Uint8Array(fallbackPixels.buffer, fallbackPixels.byteOffset, fallbackPixels.byteLength);
            }
        }
    }

    if (pixels.byteLength !== expectedBytes) {
        throw new Error(`WebGL image source ${source.width}x${source.height} provided ${pixels.byteLength} RGBA bytes; expected ${expectedBytes}`);
    }
...
function assertRgbaByteUpload(
    ...
    if (format !== gl.RGBA || type !== gl.UNSIGNED_BYTE) {
        throw new Error("Native Canvas WebGL uploads require RGBA/UNSIGNED_BYTE");
    }
}

/** Normalizes Pixi's browser image-source overloads to native RGBA uploads. */
export function installWebGlImageUploadAdapter(gl: WebGlImageUploadContext): void {
    ...
    gl.texImage2D = (...args: unknown[]): void => {
        if (args.length === 9 && isImageSource(args[8])) {
            ...
            args = [args[0], args[1], args[2], source.width, source.height, 0, args[3], args[4], readImagePixels(gl, source)];
        }
    ...
    gl.texSubImage2D = (...args: unknown[]): void => {
        if (args.length === 9 && isImageSource(args[8])) {
            ...
            args = [args[0], args[1], args[2], args[3], source.width, source.height, args[4], args[5], readImagePixels(gl, source)];
        }
