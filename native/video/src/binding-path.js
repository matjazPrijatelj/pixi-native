...
function getPlatformDirectory(platform = process.platform, arch = process.arch) {
...
function getBindingPath(platform = process.platform, arch = process.arch) {
    return path.resolve(__dirname, "..", "dist", getPlatformDirectory(platform, arch), "native_video.node");
}
