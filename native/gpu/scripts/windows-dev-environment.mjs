...
const REQUIRED_VISUAL_STUDIO_COMPONENT =
...
const findVisualStudioInstallation = () =>
  execFileSync(
    ...
    [
      ...
      "[17.0,18.0)",
      "-requires",
      REQUIRED_VISUAL_STUDIO_COMPONENT,
      "-property",
      "installationPath",
    ],
    {
...
export const initializeWindowsDevEnvironment = () => {
  if (
    ...
    (process.env.VSCMD_VER && process.env.VCToolsVersion)
  )
    return false;
  ...
  if (!Fs.existsSync(developerCommand)) {
    throw new Error(
      `Visual Studio developer command was not found: ${developerCommand}`,
    );
  }
  ...
  console.log(
    `Initialized Visual Studio developer environment from ${installationPath}`,
  );
  return true;
};
