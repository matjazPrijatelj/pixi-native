...
if (C.platform === 'win32') {
	...
	if (!Fs.existsSync(cipdBinary)) {
		...
		const digests = await Fs.promises.readFile(`${versionFile}.digests`, 'utf8')
		const digestPattern = new RegExp(`^${cipdPlatform}\\s+sha256\\s+([0-9a-f]+)$`, 'mu')
		const expectedDigest = digests.match(digestPattern)?.[1]
		if (!expectedDigest) throw new Error(`Missing CIPD SHA-256 digest for ${cipdPlatform}`)
		...
		if (actualDigest !== expectedDigest) {
			throw new Error(`Invalid CIPD SHA-256 digest: ${actualDigest} != ${expectedDigest}`)
		}
