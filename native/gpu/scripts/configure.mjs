...
const needsSync = !Fs.existsSync(
  Path.join(C.dir.dawn, ".gclient_previous_sync_commits"),
);
if (needsSync) {
  console.log("run gclient sync");
  const gclientCommand =
    C.platform === "win32"
      ? [
...
if (C.platform === "darwin") {
  ...
  backendFlags = ["-DDAWN_USE_X11=ON", "-DDAWN_USE_WAYLAND=OFF"];
} else if (C.platform === "win32") {
  backendFlags = ["-DDAWN_USE_WINDOWS_UI=ON", "-DDAWN_ENABLE_D3D12=ON"];
  ...
  const clPath = clOutput
    .split(/\r?\n/u)
    .map((line) => line.trim())
    .find(Boolean);
  if (!clPath)
    throw new Error(
      "MSVC cl.exe was not found after Visual Studio initialization.",
    );
  const msvcBin = Path.dirname(clPath);
  const linkPath = Path.join(msvcBin, "link.exe");
  if (!Fs.existsSync(linkPath))
    throw new Error(`MSVC linker was not found beside cl.exe: ${linkPath}`);
  const findWindowsTool = (name) => {
    const output = execFileSync("where.exe", [`${name}.exe`], {
      encoding: "utf8",
    });
    ...
    if (!resolved)
      throw new Error(
        `Windows SDK tool ${name}.exe was not found after Visual Studio initialization.`,
      );
    return resolved;
  };
