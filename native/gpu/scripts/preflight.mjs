...
if (process.platform === "win32") {
  ...
  if (process.arch !== "x64")
    errors.push(`Windows x64 is required; found ${process.arch}.`);

  if (!process.env.VSCMD_VER) {
    errors.push(
      "Visual Studio 2022 developer environment could not be initialized automatically.",
    );
  }
  ...
  if (vcToolsVersion.length > 0 && !isAtLeast(vcToolsVersion, [14, 41])) {
    errors.push(
      `Visual Studio 2022 v17.11 / MSVC tools 14.41 or newer are required; found ${process.env.VCToolsVersion}.`,
    );
  }
  ...
  if (windowsSdk.length === 0) {
    errors.push(
      "Windows SDK was not detected in the developer environment; install and select SDK 10.0.26100.0 or newer.",
    );
  } else if (!isAtLeast(windowsSdk, [10, 0, 26100, 0])) {
    errors.push(
      `Windows SDK 10.0.26100.0 or newer is required; found ${process.env.WindowsSDKVersion}.`,
    );
  }

  if (!commandOutput("cmake.exe", ["--version"]))
    errors.push("CMake is required in PATH.");
  if (!commandOutput("git.exe", ["--version"]))
    errors.push("Git is required in PATH.");
  ...
  if (errors.length > 0) {
    throw new Error(
      `Windows native build prerequisites are not satisfied:\n- ${errors.join("\n- ")}`,
    );
  }
