...
The demo cycles through Graphics, Sprite, normal Pixi Text, BitmapText, native video, and native audio on Windows x64 and Linux x64. It shows FPS, average frame time, and frame-time jitter in the upper-right corner and logs renderer, backend, adapter, display refresh rate, and presentation format. With the default `vsync: true`, Windows/D3D12 uses FIFO presentation and RAF is paced by DXGI's frame-latency waitable-object signal with a maximum frame latency of one; during Windows modal move/resize loops, a native window callback continues driving the Pixi ticker so GSAP and video clocks keep advancing. Other platforms and unavailable signals use a deadline timer matched to SDL's current display refresh rate. Setting `vsync: false` selects immediate WebGPU presentation and timer-paced RAF. The Sprite test uses `src/demo/assets/test-texture.png` plus generated transparent bat and platform-mascot textures, with GSAP timelines driven by the same RAF scheduler. Image and normal Text rasterization use the native Skia-backed `NodeCanvas` adapter.
