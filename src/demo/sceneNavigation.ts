const NUMBER_KEYS: Readonly<Record<string, number>> = { "1": 0, "2": 1, "3": 2, "4": 3 };

/** Returns the scene selected by a non-repeating SDL key press, or null for other keys. */
export function getSceneIndexForKey(key: string | null, currentIndex: number, sceneCount: number, repeat = false): number | null {
    if (repeat || sceneCount <= 0) return null;
    const numberedIndex = key === null ? undefined : NUMBER_KEYS[key];
    if (numberedIndex !== undefined && numberedIndex < sceneCount) return numberedIndex;
