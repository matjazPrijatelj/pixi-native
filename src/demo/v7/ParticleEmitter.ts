...
export interface ParticleEmitter7Bounds { readonly width: number; readonly height: number; }
export interface ParticleEmitter7Options { readonly random?: () => number; }
...
export class ParticleEmitter7 {
    ...
    public constructor(texture: Texture, initialBounds: ParticleEmitter7Bounds, options: ParticleEmitter7Options = {}) {
        this.texture = texture;
        this.random = options.random ?? Math.random;
        this.bounds = normalizeBounds(initialBounds);
        this.container = new ParticleContainer(MAX_PARTICLES, { position: true, rotation: true, alpha: true });
        this.container.visible = false;
    }

    public get enabled(): boolean { return this._enabled; }
    public get particleCount(): number { return this.container.children.length; }
    ...
    public update(deltaMS: number): void {
        ...
        while (this.spawnRemainder >= 1) { this.spawnRemainder--; this.spawn(); }
        for (const record of this.records) {
            if (!record.active) continue;
            record.ageMs += elapsedMS;
            if (record.ageMs >= record.lifetimeMs || record.particle.y > this.bounds.height + 24) { this.recycle(record); continue; }
    ...
    private spawn(): void {
        ...
        if (!record && this.records.length < MAX_PARTICLES) {
            record = { particle: new Sprite(this.texture), ageMs: 0, lifetimeMs: 0, velocityX: 0, velocityY: 0, spin: 0, active: false };
        ...
        record.particle.position.set(randomBetween(this.random, 20, this.bounds.width - 20), -24);
...
function normalizeBounds(bounds: ParticleEmitter7Bounds): ParticleEmitter7Bounds { return { width: Math.max(40, bounds.width), height: Math.max(40, bounds.height) }; }
function randomBetween(random: () => number, minimum: number, maximum: number): number { return minimum + random() * (maximum - minimum); }
