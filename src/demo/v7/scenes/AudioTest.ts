...
const source = fileURLToPath(new URL("../../assets/audio/howler-test.wav", import.meta.url));
...
export function createAudioTest(
    ...
    const title = new Text("NATIVE AUDIO / HOWLER + DRUM ATLAS  [6]", { fontFamily: "Arial", fontSize: 26, fill: 0xffffff });
    const instructions = new Text("U/I/O/P + J/K/L/Č or mouse click: drums  |  M: music fade-in  |  F: fade-out  |  SPACE: mute", { fontFamily: "Arial", fontSize: 18, fill: 0xffffff });
    ...
    for (const definition of DRUM_PADS) {
        ...
        const keyLabel = new Text(definition.key.toLocaleUpperCase("sl"), { fontFamily: "Arial", fontSize: 30, fill: 0xffffff });
    ...
    const sound = new Howl({
        ...
        volume: 0.65,
        preloadSprites: true,
        onload: () => { lastEvent = "music loaded"; },
        onplay: (id) => { lastEvent = `music play ${id}`; },
        onend: (id) => { lastEvent = `music loop ${id}`; },
        onfade: (id) => { lastEvent = `music fade ${id}`; },
        onplayerror: (_id, message) => { lastEvent = `music error: ${message}`; },
    });
    const drums = new Howl({
        src: [DRUM_SOURCE_PATH],
        sprite: DRUM_ATLAS.sprite,
        volume: 0.85,
        onload: () => { lastEvent = "drum MP3 atlas loaded"; },
        onplayerror: (_id, message) => { lastEvent = `drum error: ${message}`; },
    });
    ...
    scene.handleKey = (key, repeat = 0): boolean => {
        ...
        if (key === "m") {
            ...
            if (musicId === null || !sound.playing(musicId)) musicId = sound.play("music");
    ...
    scene.resize = (width, height): void => {
        ...
        const scale = Math.min(availableWidth / drumTexture.width, availableHeight / drumTexture.height);
        drumContainer.scale.set(scale);
        drumContainer.position.set((width - drumTexture.width * scale) / 2, top + (availableHeight - drumTexture.height * scale) / 2);
    };
