...
export function createRtpVideoTest(viewport: {
  width: number;
  height: number;
}): Container & {
  ...
  try {
      ...
      videos = config.urls.map(
        (url) =>
          new NativeVideo(url, {
            ...
            mediaType: "live",
            audio: false,
            reconnect: { initialDelayMs: 500, maxDelayMs: 5000 },
            ffmpeg: { inputArgs: config.inputArgs, inputPacing: "source" },
          }),
      );
    ...
    title.text = `RTP VIDEO TEST [7] | ${String(error)}`;
  }
  scene.resize = (width, height): void => {
    const cellWidth = Math.max(1, (width - 16) / 2);
    const cellHeight = Math.max(1, height - 120);
    for (let i = 0; i < sprites.length; i++) {
      const sprite = sprites[i];
      const scale = Math.min(
        cellWidth / sprite.video.width,
        cellHeight / sprite.video.height,
      );
      sprite.scale.set(scale);
      sprite.position.set(
        8 + i * (cellWidth + 8) + (cellWidth - sprite.width) / 2,
        92,
      );
    }
  };
