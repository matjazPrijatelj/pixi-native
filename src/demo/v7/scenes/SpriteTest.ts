import { Container, NineSlicePlane, Sprite, Texture } from "pixi.js-v7";
...
export function createSpriteTest(
  ...
  const nineSliceFrame = new NineSlicePlane(
    nineSliceTexture,
    NINE_SLICE_BORDER,
    NINE_SLICE_BORDER,
    NINE_SLICE_BORDER,
  ...
  const staticLayouts: StaticSpriteLayout[] = staticSprites.map(
    (sprite, index) => ({
      sprite,
  ...
  const updateTitle = (): void => {
    title.text = `SPRITE + NINE-SLICE + GSAP TEST  [2]  |  dynamic: ${dynamicSprites.length}  |  UP +10 / DOWN -10`;
  };
  ...
  const addRandomSprite = (): void => {
    ...
    dynamicSprites.push({
      ...layout,
      timeline: createDynamicAnimation(layout),
    });
  };
