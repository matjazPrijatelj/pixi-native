...
function parseBitmapFontDescriptor(source: string): BitmapFontDescriptor {
  ...
  const records = lines.map((line) => {
    ...
    for (const match of line.matchAll(ATTRIBUTE_PATTERN)) {
      ...
      attributes[match[1]] = /^-?\d+(?:\.\d+)?$/u.test(value)
        ? Number(value)
        : value;
    }
    return { type, attributes };
  });
  const find = (type: string): Record<string, string | number> =>
    records.find((record) => record.type === type)?.attributes ?? {};
  ...
  const chars = records
    .filter((record) => record.type === "char")
    .map(({ attributes }) => attributes as Record<string, number>);
  const kernings = records
    .filter((record) => record.type === "kerning")
    .map(({ attributes }) => attributes as Record<string, number>);
  return {
    ...
    common: {
      ...
      pages: Number(common.pages),
      packed: Number(common.packed),
    },
    pages,
    chars,
    kernings,
